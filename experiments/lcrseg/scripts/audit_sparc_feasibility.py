...
def _required_boundary_sizes(
    relation_features: torch.Tensor, decoder_features: dict[str, torch.Tensor]
) -> tuple[tuple[int, int], ...]:
    sizes = [tuple(relation_features.shape[-2:])]
    sizes.extend(tuple(decoder_features[layer_name].shape[-2:]) for layer_name in ("dec3", "dec1"))
    ...
    for old_index, current_index in TRANSITIONS:
        ...
        with torch.inference_mode():
            for batch_index in range(UPDATE_BATCHES):
                ...
                # Cache boundary bands at the actual frozen feature resolutions.
                # Fundus images are not required to be 256x256, so a hard-coded
                # decoder size would make the post-hoc audit input-size dependent.
                boundary_sizes = _required_boundary_sizes(
                    current_output.relation_features, current_output.decoder_features
                )
                boundary = _boundary_maps(full_labels, boundary_sizes)
                selectors = {
