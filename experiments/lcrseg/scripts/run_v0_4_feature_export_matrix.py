#!/usr/bin/env python3
"""Run one deterministic worker of the frozen V0.4 R0/R1 feature-export matrix."""
...
def _jobs(root: Path, output_dir: Path) -> list[dict[str, object]]:
    ...
    for seed in (0, 1, 2):
        for variant in ("R0", "R1"):
            run_name = RUN_NAMES[(seed, variant)]
            for site_index, trained_site in enumerate(SITE_ORDER):
                ...
                for evaluation_site in SITE_ORDER:
                    stem = (
                        f"seed{seed}_{variant}_through{site_index}-{trained_site}"
...
def main() -> None:
    ...
    for ordinal, job in enumerate(assigned, start=1):
        if _is_complete(job):
            print(f"SKIP complete {ordinal}/{len(assigned)} {Path(job['output']).name}", flush=True)
        else:
            output = Path(job["output"])
            ...
            command = [
                ...
                str(job["checkpoint"]),
                "--site",
                str(job["evaluation_site"]),
                "--seed",
                str(job["seed"]),
                "--max-pixels-per-class",
                "200000",
                "--output",
                str(output),
                "--device",
        ...
        metadata = json.loads(Path(job["output"]).with_suffix(".json").read_text(encoding="utf-8"))
        completed.append(
            {
