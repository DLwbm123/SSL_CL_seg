"""Future authorized finite AGMS P1; no production authority is shipped or inferred."""
import copy
import math
from pathlib import Path
from .protocol import STUDY,DOC,read,write,digest,canonical_plan,validate_prefix,smoke_nodes
from .authority import preflight,CAPS,Capability,_SEAL,execution_plan,baseline_environment
...
def check_costs(root,plan):
    ...
    if result['formal']>5300 or result['smoke']>8 or result['synthetic_cuda']>10 or result['formal']+result['smoke']>5308:
...
def accept_prefix(node,config,plan,permit,root,device):
    ...
    permit.validate()
    if node not in (canonical_plan()['nodes']+smoke_nodes()) or plan!=canonical_plan():raise PermissionError('canonical node required')
...
def construct(node,config,plan,permit,prefix_path,receipt,device,smoke=False):
    if not isinstance(permit,Capability):raise PermissionError('new study capability required')
    permit.validate()
    scope=permit.bindings.get('execution_scope')
    allowed=canonical_plan()['nodes'] if scope=='formal' else smoke_nodes() if scope=='smoke' else []
    if node not in allowed or plan!=canonical_plan():
    ...
    model=ObserverModel(NativeLRParent(native,node['seed'],sid),node['arm'],node['seed'],node['order'],2).to(device)
    provider=NativeCurrentDomain(config['data'],node['seed'],node['order'],2,sid,device,permit,allow_u=not smoke)
    return ObserverTrainer(model,provider,opts,node['arm'],permit,node=node)
...
def target(node,config,plan,permit,root,device,prefix_path,predecessor):
    ...
    cost=copy.deepcopy(trainer.extra_cost);telemetry=copy.deepcopy(trainer.telemetry)
    ...
    receipt=dict(node_id=node['id'],identity=ident,status='SEALED',step=node['updates'],physical_optimizer_calls=counter.count,
        ...
        origin='new_execution',scores=scores,prefix_scores=predecessor['scores'],prefix_timeline=predecessor['timeline'],
        resolved_options=plan['options'][node['domain']],diagnostics=diagnostics,support=support,extra_cost=cost,telemetry=telemetry)
