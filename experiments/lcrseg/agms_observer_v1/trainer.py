...
from .diagnostics import selection_counts, labeled_diagnostics, kl_parts
from .protocol import ARMS, METHOD, GEOMETRY, STUDY, digest
from ..agms_cl_v0_1.core import brier, selector, parent_loss, coverage, shadow_counts


class ObserverTrainer(StageTrainer):
    def __init__(self, model, provider, options, arm, execution, initialize=True, node=None):
        ...
        self.diagnostics = {}
        self.support_summary = {'active_steps': 0, 'empty_coarse': 0, 'fine': 0, 'coarse': 0, 'geometry': 0, 'U_same_state': {}}
    ...
    def losses(self):
        ...
        coarse, stats = selector(qs, geometry, fine, 'A5', self.risk)
        if not ARMS[self.arm]['H']:coarse = coarse & False
        self.last['U_same_state'] = selection_counts(qs, geometry, fine, self.risk)
        ...
        if self.step+1 in {math.ceil(total*f) for f in (.25,.5,.75,1.)}:
            with torch.no_grad():
                ...
                cl, sl = selector(qls, gl, fl, 'A5', self.risk)
                uniform, _ = selector(qls, gl, fl, 'A4' if t.aux else 'A1', self.risk)
                adaptive, _ = selector(qls, gl, fl, 'A5' if t.aux else 'A1', self.risk)
            with torch.no_grad():
                parent_kl, conditional_kl = kl_parts(logu, q, fine)
                descriptive = labeled_diagnostics(qls, y)
            self.last['diagnostic'] = self.gradient_diagnostics(supervised, kl, ds, h, ramp)
            self.last['diagnostic'].update(L_parent_conditional=descriptive, U_same_state=self.last['U_same_state'],
                KL_decomposition=dict(parent=float(parent_kl), conditional=float(conditional_kl), original=float(kl.detach()), weight=1., shadow_only=True))
    ...
    def gradient_diagnostics(self, supervised, kl, ds, h, ramp):
        ...
        vectors, norms = {}, {}
        for name, loss, enabled, factor in [('supervised',supervised,True,1.),('KL',kl,True,self.options['lambda_U']*ramp),
    ...
    def update(self, *args, **kwargs):
        ...
        if self.step != before and self.last.get('active_U') and self.arm != 'A0':
            ...
            for k,v in self.last['U_same_state'].items():s['U_same_state'][k] = s['U_same_state'].get(k,0)+v
