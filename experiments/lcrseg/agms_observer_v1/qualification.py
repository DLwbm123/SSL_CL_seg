...
def generated_trainer(reference,arm,device,permit,size=32,baseline=False):
    ...
    return ObserverTrainer(ObserverModel(parent,arm,163,1,2).to(device),provider,opts,arm,permit)
...
def baseline_equivalence(reference,device,permit,counter,size):
    values=[];head_learning=False
    for old in (True,False):
        torch.manual_seed(381);random.seed(381)
        t=generated_trainer(reference,'SHADOW',device,permit,size,baseline=old)
        initial=[p.detach().clone() for p in t.model.aux.parameters()] if not old else []
        counter.wrap(t.optimizer)
        for _ in range(2):t.update()
        v=snapshot(t,common=True)
        if not old:
            head_learning=any(not torch.equal(a,b) for a,b in zip(initial,t.model.aux.parameters()))
            for name in ('student','ema'):
                v[name]={k:x for k,x in v[name].items() if not k.startswith('aux.')}
            groups=v['optimizer']['param_groups'][:2]
            keep={p for g in groups for p in g['params']}
            v['optimizer']={'param_groups':groups,'state':{k:x for k,x in v['optimizer']['state'].items() if k in keep}}
            for key in ('base_lrs','_last_lr','lr_lambdas'):v['scheduler'][key]=v['scheduler'][key][:2]
        # Observer adds a teacher-L readout on cached features, never a backbone forward.
        assert v['telemetry'].pop('readout_only_forwards')==(0 if old else 1)
        values.append(v);del t
    exact(*values)
    assert head_learning
    return dict(main_EMA_AB_optimizer_prototypes_reads_RNG_exact=True,heads_trained=True,calls=4,status='PASS')
