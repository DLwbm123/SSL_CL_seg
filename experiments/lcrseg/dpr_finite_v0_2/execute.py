...
def child(base,tid,phase,gpu,data,reference):
    ...
    module='experiments.lcrseg.dpr_finite_v0_2.'+('engine' if phase=='train' else 'evaluate')
...
def execute(base,data,reference):
    ...
    ledger=ct.read(b/'qualification_ledger.json');assert ledger['synthetic_updates']<=ct.protocol()['qualification_synthetic_cap'] and ledger['real_smoke_updates']==8
    reservation=dict(source=source,tasks=p['tasks'],formal_updates=31800,source_training_updates=0,baseline_retraining_updates=0,
