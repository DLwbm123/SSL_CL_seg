...
def identity(node,config):
    return dict(study_id=STUDY,arm=node['arm'],dose_id=node['dose_id'],lambda_align=node['lambda_align'],family='B2_PARENT_PAS_KL',candidate_id='B2_PARENT_PAS_KL_C06',
...
def check_costs(root,plan):
    ...
    if result['formal']>42400 or result['smoke']>32 or result['synthetic_cuda']>40 or result['formal']+result['smoke']>42432:
...
def construct(node,config,plan,permit,prefix_path,receipt,device,smoke=False):
    ...
    return DoseTrainer(model,provider,opts,node['arm'],permit,node['lambda_align'],node=node)
...
def target(node,config,plan,permit,root,device,prefix_path,predecessor):
    ...
    if (nr/'latest.pt').exists():
        ...
        trainer=state.resume(nr/'latest.pt',config['reference'],device,provider,plan['options'][node['domain']],ident,permit,node['arm'],node,alignment_weight=node['lambda_align'])
