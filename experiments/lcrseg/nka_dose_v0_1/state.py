"""Native continuation as DoseTrainer, with exact auxiliary identity."""
import copy
import random
...
def metadata_path(path):return Path(str(path)+'.dose.json')


def save(t,path,identity):
    if type(t) is not DoseTrainer or t.requires_restore:raise ValueError('uncommitted/non-alignment trainer')
    for key,actual in dict(study_id=STUDY,arm=t.arm,dose_id=t.dose_id,lambda_align=t.align_weight,family=t.model.family,seed=t.provider.seed,order=t.provider.order,stage=t.provider.stage).items():
...
def validate_payload(v,meta,identity,arm,options,provider,weight):
    ...
    if (v['identity']!=identity or meta['identity']!=identity or meta['step']!=v['step']
        ...
        or nka['dose_id']!=identity['dose_id'] or nka['options']!=options or nka['prefix']!=provider.stage_source or nka['lambda_align']!=weight
...
def resume(path,reference,device,provider,options,identity,permit,arm,node=None,alignment_weight=None,meter=True):
    ...
    if scope in ('cpu_synthetic','synthetic'):
        ...
        if (scope!='formal' or node not in permit.bindings['nodes'] or options!=permit.bindings['options'][node['domain']]
            or arm!=node['arm'] or alignment_weight!=node['lambda_align'] or type(provider) is not NativeCurrentDomain
    ...
    if weight!=identity['lambda_align']:raise ValueError('dose/identity mismatch')
    ...
    t=DoseTrainer(model,provider,options,arm,permit,alignment_weight,initialize=False,
                          node=node,restored_basis=v['basis'],meter=meter)
