...
def save(t,path,identity):
    if type(t) is not AGMSTrainer or t.requires_restore:raise ValueError('uncommitted/non-AGMS trainer')
...
def resume(path,reference,device,provider,options,identity,permit,arm,node=None):
    ...
    model=AGMSModel(parent,arm,provider.seed,provider.order,provider.stage,initialize_heads=False).to(device)
    model.load_state_dict(state)
    t=AGMSTrainer(model,provider,options,arm,permit,initialize=False,node=node)
