...
def main():
    ...
    with forbid_forwards(), isolation_guard():
        ...
        status = d.read(output / "PRES_DSR_SF_STATUS.json")
        ...
        phase_path = output / "PHASE_pres_dsr_sf_MANIFEST.json"
        ...
        artifact = d.read(output / "PRES_DSR_SF_ARTIFACT_MANIFEST.json")
        ...
        audit = dict(status="PASS_PRIVATE_ARCHIVE_AUDIT", audited_at=d.now(), code_commit=args.code_commit,
                     ...
                     final_create_only_bundle_manifest="PRES_DSR_SF_PRIVATE_BUNDLE_MANIFEST.json")
        d.write_new(output / "PRES_DSR_SF_PRIVATE_ARCHIVE_AUDIT.json", audit)
        final_entries = [*entries,
                         dict(path=phase_path.name, bytes=phase_path.stat().st_size, sha256=d.sha256(phase_path)),
                         dict(path="PRES_DSR_SF_PRIVATE_ARCHIVE_AUDIT.json",
                              bytes=(output / "PRES_DSR_SF_PRIVATE_ARCHIVE_AUDIT.json").stat().st_size,
                              sha256=d.sha256(output / "PRES_DSR_SF_PRIVATE_ARCHIVE_AUDIT.json"))]
        ...
        d.write_new(output / "PRES_DSR_SF_PRIVATE_BUNDLE_MANIFEST.json", final)
