"""Create the exact-source zero-skip pytest admission receipt."""
...
def main():
    ...
    pres_cases = [case for case in root.iter("testcase") if "pres_jascl_v0_1" in case.attrib.get("classname", "")]
    require(len(pres_cases) >= 46 and totals["failures"] == totals["errors"] == totals["skipped"] == 0,
            "PRES/Gate0 test admission failed", "BLOCKED_INCOMPLETE_EVIDENCE")
    result = dict(status="PASS", code_commit=args.code_commit, publication=publication,
                  pres_test_cases=len(pres_cases), required_pres_categories=46, **totals,
