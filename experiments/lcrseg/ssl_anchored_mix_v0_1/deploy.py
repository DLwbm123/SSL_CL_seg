...
def deploy(root,data,reference,device,*,qualification=False,expected=None,shape=(384,384)):
    ...
    if p.get('head_mode')!=head_mode(p['arm']):raise PermissionError('deployment head_mode mismatch')
    m=from_state(reference,device,p.pop('student'),p['head_mode']);m.eval()
    ...
    with torch.no_grad():
        for i in range(len(ds)):
            ...
            count+=1
    result=dict(head_mode=p['head_mode'],status='PASS',source=source,models=1,student_hash=p['student_hash'],cases=count,additional_gt_reads=0,deterministic_head=True)
...
if __name__=='__main__':
    ...
    with Operations(Path(a['root']).parent/(Path(a['root']).name+'_deploy_operations')):deploy(**a,device=torch.device('cuda:0'))
