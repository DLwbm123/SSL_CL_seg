"""Shared loop for all anchored/mixed arms, adapted from the qualified predecessor."""
...
def run(data,reference,output,domain,seed,arm,device,*,expected=None,shape=(384,384),epochs=100,qualification=False,resume=False,stop_epoch=None):
    ...
    if not qualification:
        reservation=json.loads((Path(output).parents[2]/'reservation.json').read_text())
        if [seed,domain,arm] not in reservation['tasks']:raise PermissionError('task not reserved')
    ...
    if resume:
        ...
        if not qualification and seed in (31,32,33):
            old=json.loads((Path(output).parents[3]/'P0_BASELINE.json').read_text())['old_task_bindings'][f'{seed}/{domain}']
            assert init['student_hash']==old['initial_student_hash']
    ...
    if not qualification and seed in (31,32,33):
        old=json.loads((Path(output).parents[3]/'P0_BASELINE.json').read_text())['old_task_bindings'][f'{seed}/{domain}']
        assert label_order.hexdigest()==old['label_order_hash']
    ...
    result.update(labeled_unique_patients=len(l),unlabeled_unique_patients=(u_count if u is not None else 0),mix_rng='stateless keyed mix_rectangle; rectangle/source maps persisted in steps; no global stream state',checkpoint_tensor_policy='state_dict tensor references, no CPU full shadow; temporary serialization buffers separate from model count')
    write_json(root/'receipt.json',result);flush('training_complete');return result
...
if __name__=='__main__':
    ...
    a=vars(p.parse_args())
    try:
        from .telemetry import Operations
        with Operations(Path(a['output']).parent/(a['arm']+'_operations')):run(**a,device=torch.device('cuda:0'))
