...
def child(base,tid,phase,gpu,data,reference):
    ...
    module='experiments.lcrseg.cist_plasticity_v0_2.'+('engine' if phase=='train' else 'evaluate')
...
def execute(base,data,reference):
    ...
    ledger=ct.read(b/'qualification_ledger.json');assert ledger['synthetic_updates']<=96 and ledger['real_smoke_updates']==12
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as pool:
            futures=[pool.submit(worker,g) for g in (6,7)]
        ...
        for t in p['tasks']:
            ...
            students[t['task_id']]=dict(student_hash=r['student_hash'],controller_hash=r['controller_hash'],source_student_hash=r['boundary']['source_student_hash'],checkpoint_sha256=diag['checkpoint_sha256'])
