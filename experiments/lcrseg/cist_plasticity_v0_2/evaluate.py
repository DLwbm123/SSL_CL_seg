...
def evaluate(base,task_id,data,reference,device):
    ...
    seal=ct.read(b/'TARGET_WEIGHT_SEAL.json');assert len(seal['students'])==18 and seal['source']==source
    ...
    core=c.from_state(reference,device,p.pop('core'),c.LINEAR);model=c.Student(core,tensors,task).eval()
    controller=p.pop('controller')
    if model.transport is not None:model.transport.load_state_dict(controller)
    else:assert controller is None
    assert c.state_hash(model)==p['student_hash']==r['student_hash']==seal['students'][task_id]['student_hash']
    assert c.hash_state(model.fixed_state())==r['boundary']['fixed_state_hash'];del p,controller
    ...
    for domain in (task['source_domain'],task['domain']):
        ...
        for i in range(len(ds)):
            ...
            stats.append(c.mechanism(h,hp,detail));q=detail['Q'].double().cpu()[0] if detail else torch.zeros(8,8,dtype=torch.float64);bias=detail['b'].double().cpu()[0] if detail else torch.zeros(8,dtype=torch.float64);qsum+=q;bsum+=bias;q2+=float(q.square().sum());b2+=float(bias.square().sum())
        ...
        mechanisms.append(dict(task_id=task_id,domain=domain,images=n,Q_between_image_variance=max(0.,q2/n-float((qsum/n).square().sum())),b_between_image_variance=max(0.,b2/n-float((bsum/n).square().sum())),
                               ...
                               prediction_fraction_current_H_before_transport=(cls_before/pixels).tolist(),prediction_fraction_adapted=(cls_after/pixels).tolist(),confidence_current_H_before_transport=conf_before/pixels,confidence_adapted=conf_after/pixels))
    ...
    result=dict(status='COMPLETE',source=source,task_id=task_id,student_hash=r['student_hash'],models=1,seen_domains=[task['source_domain'],task['domain']],
                patient_forwards=len(rows),GT_samples=len(rows),optimizer_updates=0,backward=0,EMA_models=0,normal_deploy_core_forwards_per_image=1,normal_deploy_controller_forwards_per_image=int(model.transport is not None),
