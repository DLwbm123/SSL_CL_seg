"""One L-only loop; every dynamic core/controller parameter is recovered exactly."""
import argparse
import hashlib
import json
import os
...
def initial(base,task,reference,device,fixture=None):
    ...
    source_hash=c.state_hash(core);m=c.Student(core,tensors,task);opt=c.optimizer(m)
    boundary=dict(source_student_hash=source_hash,initial_student_hash=c.state_hash(m),fixed_state_hash=c.hash_state(m.fixed_state()),
                  trainable_names=list(m.trainable_names),trainable_parameters=sum(p.numel() for p in m.parameters() if p.requires_grad),
                  whitelist=[dict(name=n,shape=list(p.shape),scalars=p.numel()) for n,p in m.named_parameters() if p.requires_grad],
                  optimizer_state_empty=not opt.state,source_optimizer_loaded=False,source_RNG_loaded=False,EMA_models=0,full_models=audit_live_models(1))
    assert source_hash==entry['student_hash'] and boundary['optimizer_state_empty']
    return m,opt,entry,boundary

@torch.no_grad()
def diagnose(model,boundary):
    device=next(model.parameters()).device;row={}
    ...
    checks=dict(fixed_subset_hash=c.hash_state(model.fixed_state())==boundary['fixed_state_hash'],
                finite=all(bool(torch.isfinite(p).all()) for p in model.parameters()),
                fixed_gradients_absent=all(p.grad is None for p in model.parameters() if not p.requires_grad))
    if row:checks.update(orthogonality=row['orthogonality64_max']<=c.ISO64_LIMIT,distance=row['distance_relative_max']<=c.DISTANCE32_LIMIT)
    return dict(status='PASS' if all(checks.values()) else 'FAIL',checks=checks,probe=row,parameter_changes=c.drift(model),
                scope='frozen subset + synthetic geometry; actual current-H same-forward geometry recorded in training steps')
...
def train(base,task_id,data,reference,device,*,fixture=None,resume=False,stop_after=None):
    ...
    with c.training_access(task['domain']):
        ...
        if resume:
            ...
            position=p['position'];m.restore_dynamic(p['dynamic']);opt.load_state_dict(p['optimizer']);restore_rng(p['rng'],device)
        ...
        def payload(state,pos,epoch,index):
            return dict(source=source,task=task,total=total,position=pos,epoch=epoch,index=index,state=state,
                        dynamic=m.dynamic_state(),optimizer=opt.state_dict(),rng=rng_state(device),boundary=boundary,
                        ...
                        immutable_source_reference=task['source_task_id'],dynamic_keys=list(m.trainable_names),config_sha256=ct.sha256(ct.DOC/'PROTOCOL.json') if fixture is None else 'SYNTHETIC')
        for ei,epoch in enumerate(epochs):
            ...
            for index,ii in enumerate(lis):
                ...
                if m.transport is not None:m.transport.journal=journal
                lb=c.batch(ds,ii,device,task['seed'],epoch,index,'labeled')
                try:row=c.step(m,opt,lb,task,epoch,index,counts,[patients[i] for i in ii])
                finally:
                    if m.transport is not None:m.transport.journal=None
        ...
        assert counts['optimizer_steps']==total and c.hash_state(m.fixed_state())==boundary['fixed_state_hash']
        controller_hash=c.state_hash(m.transport) if m.transport is not None else None;student_hash=c.state_hash(m)
        save(root/'deploy_student.pt',dict(core=m.core.state_dict(),controller=m.transport.state_dict() if m.transport is not None else None,student_hash=student_hash,
        ...
        memory=dict(full_models=audit_live_models(1),core_parameter_bytes=c.tensor_bytes(dict(m.core.named_parameters())),
                    ...
                    trainable_bytes=c.tensor_bytes([p for p in m.parameters() if p.requires_grad]),
                    fixed_parameter_bytes=c.tensor_bytes([p for p in m.parameters() if not p.requires_grad]),source_reference_CPU_bytes=c.tensor_bytes(m.initial_values),
                    controller_parameters=sum(p.numel() for p in m.transport.parameters()) if m.transport is not None else 0,controller_parameter_bytes=c.tensor_bytes(dict(m.transport.named_parameters())) if m.transport is not None else 0,
                    basis_buffer_bytes=c.tensor_bytes(dict(m.named_buffers())),Adam_bytes=c.tensor_bytes(opt.state_dict()),
                    gradient_bytes=c.tensor_bytes([p.grad for p in m.parameters() if p.grad is not None]),EMA_bytes=0,
        ...
        result=dict(status='TRAINING_COMPLETE',source=source,task=task,updates=position,student_hash=student_hash,controller_hash=controller_hash,
                    ...
                    L_opens=prior_l+ds.opens,U_opens=prior_u+(u.opens if u else 0),fixed_subset_unchanged=True,EMA_updates=0,pseudo_labels=0,
