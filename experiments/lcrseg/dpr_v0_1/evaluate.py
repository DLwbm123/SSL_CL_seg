...
from .contract import verify,admit,read,neutral_subprocess_paths
...
def evaluate(base,task_id,data,reference,device,fixture=None):
    ...
    root=Path(base)/'tasks'/task_id;out=root/'evaluation'
    if fixture is None:
        seal=read(Path(base)/'TARGET_WEIGHT_SEAL.json')
        assert seal['source']==source and len(seal['students'])==18
        assert str(Path(data).resolve())==read(Path(base)/'private_inputs.json')['data']
    out.mkdir()
    ...
    if fixture is None:assert receipt['student_hash']==seal['students'][task_id]['student_hash']
...
if __name__=='__main__':
    ...
    a=vars(p.parse_args());neutral_subprocess_paths();root=Path(a['base'])/'tasks'/a['task_id']
