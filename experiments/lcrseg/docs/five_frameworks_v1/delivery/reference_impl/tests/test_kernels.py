...
def test_repair_zero_shape_is_exact_identity():
    h=torch.randn(1,5,5,5,dtype=DT,requires_grad=True)
    q,_=torch.linalg.qr(torch.randn(5,2,dtype=DT));c=torch.randn(3,5,1,1,dtype=DT)
    out,stats=repair_in_subspace(h,q,lambda v:F.conv2d(v,c),lambda p:p[:,2].square().mean(),shape_weight=0)
    assert torch.equal(out,F.conv2d(h.detach(),c).softmax(1))
    assert not out.requires_grad and h.grad is None and stats['coordinate_vjps']==0


def test_repair_detached_and_finite_with_toy_penalty():
    h=torch.randn(1,5,5,5,dtype=DT,requires_grad=True)
    q,_=torch.linalg.qr(torch.randn(5,2,dtype=DT));c=torch.randn(3,5,1,1,dtype=DT)
    out,stats=repair_in_subspace(h,q,lambda v:F.conv2d(v,c),lambda p:p[:,2].square().mean(),shape_weight=.1)
    assert not out.requires_grad and h.grad is None and stats['coordinate_vjps']==3
    assert stats['readout_forwards']==5 and torch.isfinite(out).all()
    assert torch.allclose(out.sum(1),torch.ones_like(out[:,0]),atol=1e-12)
