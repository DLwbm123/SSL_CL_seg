...
No real-data loaders, optimizer runner, CWMI backend or D-Convexity reproduction
are included. All teacher targets, masks and spectral scales are stop-gradient.
...
def uncertainty_scales(uncertainty: Tensor, eigenvalues: Tensor, kappa: float,
    ...
    return (1 + kappa * uncertainty.detach()[:, None] /
            s.clamp_min(eps)[None, :, None, None]).reciprocal()
...
def repair_in_subspace(feature: Tensor, q_basis: Tensor,
                       readout: Callable[[Tensor], Tensor],
                       shape_loss: Callable[[Tensor], Tensor], *,
                       shape_weight: float, steps: int = 3,
                       step_size: float = 0.05, trust_fraction: float = 0.1,
                       l2_weight: float = 0.1) -> tuple[Tensor, dict]:
    """Detached teacher target repair. readout must use frozen functional weights.

    shape_loss is an injected verified D-Convexity adaptation in production.
    Synthetic tests inject a toy differentiable penalty and do NOT verify that
    third-party loss. Fixed projected, RMS-normalized gradient steps; no claim
    of exact minimization/convexity or global reachability.
    """
    if feature.ndim != 4 or q_basis.ndim != 2 or feature.shape[1] != q_basis.shape[0]:
        raise ValueError("feature/basis mismatch")
    if steps < 0 or min(shape_weight, step_size, trust_fraction, l2_weight) < 0:
        raise ValueError("negative repair option")
    h, q = feature.detach(), q_basis.detach().to(feature)
    with torch.no_grad():
        base = readout(h).softmax(1).detach()
        radius = trust_fraction * h.square().mean(1, keepdim=True).sqrt().clamp_min(1e-6)
    if shape_weight == 0 or steps == 0:
        return base, {"steps": 0, "readout_forwards": 1, "coordinate_vjps": 0, "objectives": [], "coordinate_norm_max": 0.0}
    d = h.new_zeros(h.shape[0], q.shape[1], *h.shape[2:])
    objectives = []
    for _ in range(steps):
        d = d.detach().requires_grad_(True)
        with torch.enable_grad():
            logits = readout(h + torch.einsum('dk,bkhw->bdhw', q, d))
            probs = logits.softmax(1)
            valid = torch.ones_like(base[:, 0], dtype=torch.bool)
            fidelity = masked_kl(logits, base, valid)
            penalty = shape_loss(probs)
            objective = fidelity + shape_weight * penalty + l2_weight * d.square().mean()
            _finite(objective, "repair objective")
            grad, = torch.autograd.grad(objective, d, create_graph=False)
        objectives.append(float(objective.detach()))
        with torch.no_grad():
            rms = grad.square().mean((1, 2, 3), keepdim=True).sqrt().clamp_min(1e-12)
            d = d - step_size * radius * grad / rms
            norm = d.norm(dim=1, keepdim=True).clamp_min(1e-12)
            d = d * torch.minimum(torch.ones_like(norm), radius / norm)
    with torch.no_grad():
        result = readout(h + torch.einsum('dk,bkhw->bdhw', q, d.detach())).softmax(1)
    return result.detach(), {"steps": steps, "readout_forwards": steps + 2,
                             "coordinate_vjps": steps, "objectives": objectives,
                             "coordinate_norm_max": float(d.detach().norm(dim=1).max())}
