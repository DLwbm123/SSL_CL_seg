...
The synthetic parent is deterministic with no BN or stochastic classifier. These tests do not verify unknown real stochastic-layer behavior, hard/soft KI rules, geometry, native Dice, source identity, loader permissions or CUDA/real AMP. These remain binding/qualification items rather than being mislabeled skips or passes. Rank-insufficient fallback is tested at both kernel and F2 caller boundaries; malformed projectors/gradients propagate and preserve all A tensors.

R2 adds the unchanged nine external regressions and tests in test_revision_r2.py: all ten arms independently resume mid-stage 2 with exact complete state/trajectory comparison; semantic/provider/source mismatches reject before restore mutation; numeric optimizer/constraint/scheduler/EMA transaction checks; typed F2 errors; behaviorally distinct modes for all five frameworks; ignore support; native callback/data schema/preflight/DAG metadata checks; device-local CWMI CPU parity. Test-only cost instrumentation covers the main process and all ten resume workers; production code does not import it.

R3 responds only to external R2 issue R07. The unchanged external eleven tests and five additional ownership cases bring the full suite to 178. Coverage adds original L/U row and list replacement, nested geometry/callback mutation, public snapshot copies, permit allowed-digest/budget/phase immutability and seal identity, and native metadata/whitelist ownership with JSON serialization and module deepcopy preservation. Earlier empty-registry/manual-JSON launch rejection tests remain unchanged and pass. No reviewed-source snapshot helper is used in repository tests.
