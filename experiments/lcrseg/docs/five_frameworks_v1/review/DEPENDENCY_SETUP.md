...
CWMI uses installed torch, torchvision, numpy, matplotlib and Pillow due to the author's imports. Version evidence is in the lock. No environment creation or torch reinstallation is required. R2 implements per-device factories/cache without moving input tensors through CPU; CUDA and formal optimizer qualification are still NOT_RUN. Other kernels have CPU evidence only; absence of a CUDA test is never treated as a CUDA pass.
