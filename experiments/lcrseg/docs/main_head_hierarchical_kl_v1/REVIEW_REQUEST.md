...
`p0_b_runner.py` remains a default-deny metadata gate. It now preserves duplicate role counts, validates per-state and aggregate accounting, and binds the actual PLAN/budget bytes before payload access. `execution.py` adds the single P0 dispatch path with an injectable `NativeBackend`; `execution_tests.py` drives that same path through a clearly synthetic callback backend for exactly 84 counted forwards. No production approval is present, so no payload is opened.
