...
from experiments.lcrseg.dpr_finite_v0_2 import contract as ct,results as r,execute as ex
...
def run():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        b=Path(tmp);inputs=dict(parent=str(b/'parent'),baseline_original=str(b/'old'),baseline_dpr=str(b/'old'));sb=dict(sources={});bb=dict(baselines={});students={}
        def score(path,tid,arm,domains,src):
            rows=[];summary=[]
            for domain in domains:
                v=.68 if arm=='SRC_CE' or (arm=='STATIC_SOURCE' and domain==src) else (.5 if domain==src else .7)
                v+={'DPR_FINITE_U':.01,'DPR_FINITE_SIGN_U':.003,'DPR_U':.005}.get(arm,0)
        ...
        put(q/'qualification_ledger.json',dict(synthetic_updates=0,real_smoke_updates=8))
        dispatched=[]
        ...
        assert len(dispatched)==24 and len(set(dispatched))==24
        seal=json.loads((q/'TARGET_WEIGHT_SEAL.json').read_text());assert len(seal['students'])==12
    result=dict(status='PASS',scope='synthetic report and executor fixture only; no model or data',formal_updates=0,synthetic_optimizer_updates=0,report_branches=3,queue_dispatches=24)
