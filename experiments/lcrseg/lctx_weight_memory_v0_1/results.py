...
def finish(base,recovery=False):
    if recovery:
        from experiments.lcrseg.lctx_weight_memory_v0_1_1 import contract as rc
    b=Path(base);source=rc.verify() if recovery else verify();p=protocol();binding=read(b/'SOURCE_BINDING_AND_LAYER_MANIFEST.json');parent=Path(read(b/'private_inputs.json')['parent']);out=b/'public_results';out.mkdir()
    ...
    for task in p['tasks']:
        tid=task['task_id'];root=rc.task_root(b,tid) if recovery else b/'tasks'/tid;r=read(root/'receipt.json');ev=read(root/'evaluation/receipt.json')
        training_source=rc.OLD_SOURCE if recovery and tid in read(b/'RECOVERY_MANIFEST.json')['reused_task_ids'] else source
        assert r['source']==training_source and r['task']==task and r['updates']==task['updates'] and r['status']=='TRAINING_COMPLETE'
        assert seal['students'][tid]==r['student_hash']==ev['student_hash'] and ev['source']==training_source and ev['models']==1 and r['memory']['models']==2
        ...
        for phase in ['train','eval']:
            assert read(root.parent.parent/'logs'/(tid+'_'+phase+'_exit.json'))['exit_code']==0
            oc=read(root.parent/(tid+('_operations' if phase=='train' else '_eval_operations'))/'operation_counts.json');assert oc['status']=='PASS';ops.update(oc['counts'])
        ...
        diagnostic_rows=read(b/'reaudit'/(tid+'.json'))['rows'] if recovery and training_source==rc.OLD_SOURCE else read(root/'PARAMETER_DIAGNOSTICS.json')
        for row in diagnostic_rows:
            meta=next(z for z in entry['layers'] if z['layer']==row['layer']);parameters.append(dict(row,task_id=tid,seed=task['seed'],order=task['order'],k=meta['k'],rank=8,selected_source_energy=meta['selected_energy'],parameterized_right_leak=row.get('parameterized_right_leak'),parameterized_left_leak=row.get('parameterized_left_leak'),training_source_commit=training_source,reaudit_source_commit=source if recovery else None))
        counts=Counter();n=0
    ...
    if recovery:
        from experiments.lcrseg.lctx_weight_memory_v0_1_1.closeout import supplement
        terminal=supplement(b,terminal,v,costs,ops)
    return terminal
