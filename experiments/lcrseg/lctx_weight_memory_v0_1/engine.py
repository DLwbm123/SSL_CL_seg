...
def train(base,task_id,data,reference,device,*,resume=False,fixture=None,stop_after=None):
    source=verify() if fixture is None else 'SYNTHETIC';b=Path(base);task=admit(b,task_id,source) if fixture is None else fixture['task']
    if fixture is None:
    ...
    with c.training_access(task['domain']):
        ...
        if resume:
            ...
            diag=c.diagnostics(m,load,0,task['arm']);c.write_json(root/'boundary.json',dict(status='PASS',source=source,**boundary))
        ...
        for epoch in range(position//steps+1,epochs+1):
            ...
            for index,ii in enumerate(lis):
                ...
                if index==steps-1 or position==stop_after:
                    ...
                    save(root/'latest.pt',dict(task=task,source=source,total=total,position=position,student=m.state_dict(),EMA=t.state_dict(),optimizer=opt.state_dict(),counts=dict(counts),boundary=boundary,L_opens=priorL+ds.opens,rng=rng,diagnostics=diag))
...
if __name__=='__main__':
    ...
    p.add_argument('--resume',action='store_true');a=vars(p.parse_args());root=Path(a['base'])/'tasks'/a['task_id'];op=Operations(root.parent/(root.name+'_operations'))
    if a['resume']:
