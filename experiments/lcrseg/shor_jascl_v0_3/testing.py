"""Create exact-source zero-skip SHOR admission receipt."""
...
def main():
    ...
    cases = [case for case in root.iter("testcase") if "shor_jascl_v0_3" in case.attrib.get("classname", "")]
    require(len(cases) >= 46 and totals["failures"] == totals["errors"] == totals["skipped"] == 0,
            "SHOR/PRES regression admission failed", "BLOCKED_INCOMPLETE_EVIDENCE")
    result = dict(status="PASS", code_commit=args.code_commit, publication=publication,
                  shor_test_cases=len(cases), required_shor_categories=46, **totals,
