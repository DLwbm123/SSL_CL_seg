...
def child(base,tid,phase,gpu,data,reference):
    ...
    module='experiments.lcrseg.cist_v0_1.'+('engine' if phase=='train' else 'evaluate')
...
def execute(base,data,reference):
    ...
    ledger=ct.read(b/'qualification_ledger.json');assert ledger['synthetic_updates']<=128 and ledger['real_smoke_updates']==24
    reservation=dict(source=source,tasks=p['tasks'],formal_updates=95400,source_training_updates=0,baseline_retraining_updates=0,
    ...
    (b/'logs').mkdir();jobs=queue.Queue();stop=threading.Event();minimum=ct.read(b/'smoke/receipt.json')['admission_mib']
    ...
    def worker(gpu):
        try:
            while not stop.is_set() and not jobs.empty():
                ...
                child(b,tid,'train',gpu,data,reference);child(b,tid,'eval',gpu,data,reference)
        except BaseException:
            stop.set();raise
    try:
        with ThreadPoolExecutor(max_workers=4) as pool:
            futures=[pool.submit(worker,g) for g in (5,4,6,7)]
            for f in futures:f.result()
        if stop.is_set():raise RuntimeError('new dispatch stopped after engineering failure')
        students={}
        for t in p['tasks']:
            root=b/'tasks'/t['task_id'];r=ct.read(root/'receipt.json');ev=ct.read(root/'evaluation/receipt.json');diag=ct.read(root/('diagnostics_pass_'+str(t['updates'])+'.json'))
            assert r['source']==ev['source']==source and r['task']==t and r['updates']==t['updates'] and diag['status']=='PASS'
            assert r['student_hash']==ev['student_hash'];students[t['task_id']]=dict(student_hash=r['student_hash'],controller_hash=r['controller_hash'],source_student_hash=r['boundary']['source_student_hash'],checkpoint_sha256=diag['checkpoint_sha256'])
