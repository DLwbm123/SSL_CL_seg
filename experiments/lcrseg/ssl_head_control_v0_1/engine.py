"""One loop for the four explicitly authorized head-control recipes; fixed snapshots never select weights."""
...
def run(data,reference,output,domain,seed,arm,device,*,expected=None,shape=(384,384),epochs=100,qualification=False,resume=False,stop_epoch=None):
    ...
    if not qualification and (seed not in (11,21,22) or epochs!=100 or stop_epoch is not None):raise PermissionError('fixed matrix')
